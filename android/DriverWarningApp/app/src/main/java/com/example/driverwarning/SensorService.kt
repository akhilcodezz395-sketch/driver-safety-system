...
class SensorService : Service(), SensorEventListener {
    ...
    private fun runInference(window: List<SensorSample>) {
        ...
        try {
            ...
            Log.d(TAG, "Normalized: ${normalizedFeatures.contentToString()}")
            ...
            // Show toast on first prediction
            if (firstPrediction) {
                firstPrediction = false
                android.os.Handler(android.os.Looper.getMainLooper()).post {
                    android.widget.Toast.makeText(
                        this@SensorService, 
                        "🎯 Predictions working! ${classNames[predictedClass]}", 
                        android.widget.Toast.LENGTH_LONG
                    ).show()
                }
            }
            
            // Broadcast prediction update to MainActivity
            val intent = Intent(MainActivity.ACTION_PREDICTION_UPDATE).apply {
